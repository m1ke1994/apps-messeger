  ...
  <div
    ...
    <div class="flex items-center gap-3 p-4">
      <div class="relative flex-1">
        <!-- иконка поиска слева -->
        <svg class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 opacity-60"
          xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 256 256">
          <path
            d="M229.66,218.34l-50.07-50.07a88.11,88.11,0,1,0-11.31,11.31l50.07,50.07a8,8,0,0,0,11.31-11.31ZM40,112a72,72,0,1,1,72,72A72.08,72.08,0,0,1,40,112Z" />
        </svg>
        <input v-model.trim="searchQuery" @keyup.enter="applyFilters()" type="text" placeholder="Поиск заданий"
    ...
    <main class="flex-grow">
      <div class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2 lg:grid-cols-3">
        <article v-for="t in filteredTasks" :key="t.id"
          ...
          <div class="flex items-start justify-between gap-3 px-4 pt-4">
            <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[11px] font-semibold"
              ...
              <span class="block h-1.5 w-1.5 rounded-full"
                :style="{ backgroundColor: statusColors[t.status] || '#6b7280' }"></span>
              {{ statusLabels[t.status] || t.status }}
            </span>
            <span v-if="t.price != null"
              class="rounded-full bg-primary/15 px-3 py-1 text-sm font-extrabold text-primary backdrop-blur">
          ...
          <div class="px-4 pb-4 pt-3">
            <h3
              class="mb-2 text-base font-extrabold leading-snug text-text-light transition group-hover:text-primary dark:text-text-dark">
              {{ t.title }}
            </h3>
            ...
            <div class="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
              ...
              <span v-else-if="t.deadlineAt" class="rounded-full bg-slate-100 px-2.5 py-1 dark:bg-white/10">
            ...
            <!-- Разделитель -->
            <div
              class="my-4 h-px w-full bg-gradient-to-r from-transparent via-black/10 to-transparent dark:via-white/10">
            </div>
          ...
          <div class="pointer-events-none absolute inset-0 opacity-0 transition group-hover:opacity-100"
            aria-hidden="true">
            <div
              class="absolute -inset-px rounded-[1.05rem] bg-[radial-gradient(120px_120px_at_var(--mx,50%)_-10%,rgba(19,236,164,.25),transparent)]">
            </div>
          </div>
        </article>

        <p v-if="!filteredTasks.length"
          class="col-span-full rounded-2xl border border-dashed border-gray-300 bg-white/60 p-6 text-center text-sm font-medium text-slate-600 dark:border-white/10 dark:bg-white/5 dark:text-slate-300">
    ...
    <transition name="fade">
      <div v-if="showFilter" class="fixed inset-0 z-40" @click.self="closeFilterPanel()">
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>
        <aside class="absolute right-0 top-0 z-50 h-full w-[85%] max-w-sm translate-x-0
         bg-white dark:bg-slate-900
         border-l border-black/10 dark:border-white/10
         p-4 shadow-xl" role="dialog" aria-modal="true" aria-labelledby="filters-title"
          @click.self="closeFilterPanel()">
          <!-- Заголовок -->
          <div class="mb-3 flex items-center justify-between">
            <h2 id="filters-title" class="text-base font-extrabold text-slate-900 dark:text-slate-100">
              Фильтры
            </h2>
            <button @click="closeFilterPanel()" type="button" aria-label="Закрыть" class="btn-icon">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 256 256" fill="currentColor">
                <path
                  d="M205.66,194.34a8,8,0,0,1-11.32,11.32L128,139.31,61.66,205.66a8,8,0,0,1-11.32-11.32L116.69,128,50.34,61.66A8,8,0,1,1,61.66,50.34L128,116.69l66.34-66.35a8,8,0,0,1,11.32,11.32L139.31,128Z" />
              </svg>
            </button>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              <label for="filter-category"
                class="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-600 dark:text-slate-300">
                Категория
              </label>
              <select id="filter-category" v-model="tempCategory" class="w-full rounded-xl border border-black/10 bg-white/80 p-2.5 text-sm text-slate-900
               outline-none transition
               focus:border-[#55E792] focus:ring-2 focus:ring-[#55E792]/40
            ...
            <div>
              ...
              <select id="filter-status" v-model="tempStatus" class="w-full rounded-xl border border-black/10 bg-white/80 p-2.5 text-sm text-slate-900
               outline-none transition
               focus:border-[#55E792] focus:ring-2 focus:ring-[#55E792]/40
               dark:border-white/10 dark:bg-white/5 dark:text-slate-100">
...
body {
  min-height: max(var(--mobile-h), 100dvh);
}
